...
CreateThread(function() -- Don't mind this, once the player connects it will no longer loop.
	while (true) do
		...
		if NetworkIsPlayerActive(PlayerId()) then
			Wait(500)
			TriggerServerEvent('liam-pedmenu:server:player-connected')
			break
		end
    end
end)

RegisterNetEvent('liam-pedmenu:client:player-connected', function(s_data)
    data.peds = s_data[1]
    data.donator = s_data[2]
end)

RegisterNetEvent('liam-pedmenu:client:show-notification', function(text)
    notification(text)
end)

RegisterNetEvent('liam-pedmenu:client:update-perms', function(bool)
