function showError(text)
    ...
    print(('^4[Ped-Menu] ^1[ERROR]^0: %s\n^4[Ped-Menu] ^3[Support]^0: discord.gg/liam'):format(text))
end
...
RegisterNetEvent('liam-pedmenu:server:player-connected', function()
    ...
    TriggerClientEvent('liam-pedmenu:client:player-connected', src, {config?.peds, has_permission})
end)
...
if (config?.discord?.enabled and config?.discord?.refresh?.enabled) then
    RegisterCommand(config?.discord?.refresh?.command, function(source)
        if (cooldown[source]) then
            TriggerClientEvent('liam-pedmenu:client:show-notification', source, ('You must ~r~wait~s~ %s seconds before using command again.'):format(cooldown[source]))
            return
        end
        ...
        TriggerClientEvent('liam-pedmenu:client:update-perms', source, has_permission)
